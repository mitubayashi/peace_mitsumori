...
//			<link rel="stylesheet" href="./MonthPicker.css">';
			...
			<link rel="stylesheet" href="./jquery.datetimepicker.css">
			<link rel="stylesheet" href="./MonthPicker.css">';
		$html .= '<link rel="icon" type="image/png" href="./image/favicon.ico">';
		return $html;
	...
	function makeScriptPart()
	{
		$html ='<!---↓jQuery---!>
            ...
            <script src="./kagenForm.js"></script>';
	...
	function makeformInsert_setV3( &$post, $columns_string, $out_err_string, $readonly_string, $form_name ,&$param_setting)
	{
		...
		for($i = 0 ; $i < count($columns_array) ; $i++)
		{
			...
			//HTML
            //2022-03-17 自社マスタメンテ(振込先を複数登録に変更)
            if($colum == "jsyTRANSFER")
            {
                //振込手数料欄作成
                $input_result[0] = transfer_type_radio($param_setting,$post, $colum);
                        
                //振込先入力欄作成
                $input_result[0] .=  kagenFormHtml($param_setting, $colum);
                $input_result[1] = "";
            }
            else
            {
	...
	function getFormHtml(&$form_ini, $colum, &$post, $isnotnull, $form_name, $readonly_setting)
	{
		...
		if($form_value == "")
		{
            ...
            if($colum == "sehTRANSFER")
            {
                $form_value = default_transter($form_value);
            }
		}
...
//				$javascript_str .= "$('#$element_id').datepicker();";
        ...
        if($colum == "sehTRANSFER")
        {
            $pulldown = pulldown_set($colum,$post);
        }
        else
        {
            $pulldown = "";
 ...
 * 加減式フォーム表示
 */
function kagenFormHtml(&$form_ini, $colum)
{
    //DB接続
    $con = dbconect();
    $tra_row = array();
    ...
    //項目名
    $input_html = '<td class = "space"></td><td class ="one" style="vertical-align: top;">';
    $input_html .= '<a class = "itemname">振込先※</a></td>';
         
    $input_html .= '<td class ="two">';
    $input_html .= '<table id="kagentable">';
    
    if(isset($_POST["form_traTRANSFER"]))
    {
        for($i = 0; $i < count($_POST["form_traTRANSFER"]); $i++)
        {
            $tra_row[] = $_POST["form_traTRANSFER"][$i];
            $id[] = $_POST["form_traTRANSFER_id"][$i];
        }
    }
    else
    {
        //SQL作成
        $sql = "SELECT *FROM hurikomisakiinfo;";
        //クエリ発行
        $result = $con->query($sql) or ($judge = true);   
        while($result_row = $result->fetch_array(MYSQLI_ASSOC))
        {
            $tra_row[] = $result_row["TRANSFER"];
            $id[] = $result_row["TRAID"];
        }
    }            

    for($i = 0; $i < count($tra_row); $i++)
    {
        $input_html .= '<tr><td>';
//        $input_html .= '<textarea name="form_traTRANSFER[]" id="" cols="70" rows="3" class="form_traTRANSFER[]" onchange="kagenformcheck();">';
//        $input_html .= $tra_row[$i];
//        $input_html .=  '</textarea>';
        $input_html .= '<input type="text" value="'.$tra_row[$i].'" size="50" name="form_traTRANSFER[]" class="form_traTRANSFER[]" onchange="kagenformcheck();">';
        $input_html .= '<input type="hidden" name="form_traTRANSFER_id[]" value="'.$id[$i].'">';
        $input_html .= '</td>';
        $input_html .= '<td><input type="button" value="削除" onclick="delete_textbox(this);"></td></tr>';
    }
    $input_html .= "</table>";
    $input_html .= '<input type="button" value="振込先追加" onclick="insert_textbox();">';
    $input_html .= '<span id="kensu">'.$i.'行入力済み</span>';
    $input_html .= '</td>';
    return $input_html;
}

function transfer_type_radio(&$form_ini,$post,$colum)
{    
    //項目名
    $input_html = '<td class = "space"></td><td class ="one">';
    $input_html .= '<a class = "itemname">';
    $input_html .= $form_ini[$colum]['item_name'];
    $input_html .= '</a></td>';
         
    $input_html .= '<td class ="two">';
    $input_html .= '文言なし<input type="radio" name="form_'.$colum.'_0" value="0"';
    if($post["form_jsyTRANSFER_0"] == "0")
    {
        $input_html .= ' checked';        
    }
    $input_html .= '>';
    
    $input_html .= '　　お客様負担<input type="radio" name="form_'.$colum.'_0" value="1"';
    if($post["form_jsyTRANSFER_0"] == "1")
    {
        $input_html .= ' checked';        
    }
    $input_html .= '>';
...
function pulldown_set($colum,$post)
{
    $con = dbconect();
    //SQL作成
    $sql = "SELECT *FROM hurikomisakiinfo;";
    //クエリ発行
    $result = $con->query($sql) or ($judge = true);
    if($result->num_rows == 1 && !isset($_GET["edit_list_id"]))
    {
        $html = "";
    }
    else
    {
        $html = '<td><select id="'.$colum.'_pulldown" name="'.$colum.'_pulldown">';
        while($result_row = $result->fetch_array(MYSQLI_ASSOC))
        {
            if(isset($post[$colum."_pulldown"]) && $post[$colum."_pulldown"] == $result_row['TRANSFER'])
            {
                $select = " selected";
            }
            else
            ...
            $html .= '<option'.$select.'>'.$result_row['TRANSFER'].'</option>';
        }
        $html .= '</select></td>';
        $html .= "<td><input type='button' value='変更' onclick='select_pulldown();'>";

        //振込先手数料取得
        $sql = "SELECT TRANSFER FROM jisyamaster;";
        $result = $con->query($sql) or ($judge = true);
        while($result_row = $result->fetch_array(MYSQLI_ASSOC))
        {
            $transfer_type = $result_row["TRANSFER"];
        }

        $html .= "<input type='hidden' id='transfer_type' value='".$transfer_type."'></td>";
    }
    return $html;
}