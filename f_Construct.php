 ...
 * @retrun デフォルト振込先名
 */
function default_transter($form_value)
{
    //デフォルト振込先取得
    $con = dbconect();
    $sql = "SELECT * FROM hurikomisakiinfo where TRAID = '1';";
    $result = $con->query($sql) or ($judge = true);
    while($result_row = $result->fetch_array(MYSQLI_ASSOC))
    {
        $transfer = $result_row['TRANSFER'];
    }
    
    //デフォルト振込先作成
    $default_transfer = "お振込のご案内\nお手数ですが下記口座へお振込願います。\n";

    if($form_value == "1")
    {
        $default_transfer .= "なお、振込手数料はお客様にてご負担ください。\n";
    }
