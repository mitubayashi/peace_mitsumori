...
//        $print .= "</div>";
        ...
        //案件名追加
        if ($this->filename ==="MITSUMORIPRINT_5") {
            $post = $this->prContainer->pbInputContent;
            $print .= "<div style='float: left; width: 100%;'>";
            $print .= "<span style='display: block;margin-top: 1%; margin-left: 2%;'>下記の通りお見積り申し上げます。</span>";
            $print .= "<div class='ankenmei'>件名　　　".$post["form_mmhKENMEI_0"]."</div>";
            $print .= "</div>";
        }
        if($this->filename ==="SEIKYUPRINT_5")
        {
            $post = $this->prContainer->pbInputContent;
            $print .= "<div style='float: left; width: 100%;'>";
            $print .= "<span style='display: block;margin-top: 1%; margin-left: 2%;'>下記の通りご請求申し上げます。</span>";
            $print .= "<div class='ankenmei'>件名　　　".$post["form_sehKENMEI_0"]."</div>";
            $print .= "</div>";
        }
        //金額、有効期限、自社情報、印鑑部分の枠
        if ($this->filename ==="MITSUMORIPRINT_5" || $this->filename ==="SEIKYUPRINT_5") {
            //$style = "style='height: 82%;'";
...
//                $suryo_print = number_format($suryo, 2);
        ...
        if ($this->filename === "MITSUMORIPRINT_5" && $flg === 1) {
            $print .= "<div class='goannaiBox' style='height: 6%;'>";
