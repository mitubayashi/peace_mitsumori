...
function hurikomi_edit()
{
    ...
    while($result_row = $result->fetch_array(MYSQLI_ASSOC))
    {
        $id_array[] = $result_row['TRAID'];
    }

    //余分なデータを削除する
    $id_diff = array_diff($id_array, $_POST["form_traTRANSFER_id"]);
    $delete_array = array_merge($id_diff);
    for($i = 0; $i < count($delete_array); $i++)
    {
        $delete_sql = "DELETE FROM hurikomisakiinfo WHERE TRAID = '".$delete_array[$i]."';";
        $result = $con->query($delete_sql) or ($judge = true);
    }
    
    //振込先更新
    $counter = 1;
    for($i = 0; $i < count($_POST["form_traTRANSFER_id"]); $i++)
    {
        if($_POST["form_traTRANSFER_id"][$i] == "")
        {
            $sql = "INSERT INTO hurikomisakiinfo (TRAID,TRANSFER) value('".$counter."','".$_POST["form_traTRANSFER"][$i]."');";
        }
        else
        {
            $sql = "UPDATE hurikomisakiinfo set TRAID = '".$counter."', TRANSFER = '".$_POST["form_traTRANSFER"][$i]."' WHERE TRAID = '".$_POST["form_traTRANSFER_id"][$i]."';";                                
        }
        $result = $con->query($sql) or ($judge = true);
        $counter++;
    }
