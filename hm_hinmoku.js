	...
	$(ctrlname).autocomplete(
	{
		...
		select : function(e, ui)
		{
                    ...
                    {       
                        ...
                        //入力値を取得する
                        var def_hinmei = document.getElementById(HINMEI_NAME+poststr).defaultValue;
                        var def_tanka = document.getElementById(TANKA_NAME+poststr).defaultValue;
                        var def_suryo = document.getElementById(SURYO_NAME+poststr).defaultValue;
                        var def_tanni = document.getElementById(TANNI_NAME+poststr).defaultValue;
                        
                        //入力値を取得する
                        var hinmei = document.getElementById(HINMEI_NAME+poststr).value;
                        
                        if(def_hinmei == '' && def_tanka == '' && def_suryo == '' && def_tanni == '')
                        {
                            document.getElementById(HINMEI_NAME+poststr).defaultValue = ui.item.value;
                            $('#' + ZEIRISTU_NAME + poststr).val(ui.item.ZEI);
                        }
                        else
                        {
                            if($('#'+ZEIRISTU_NAME+poststr).val() != 10 && hinmei != '以下余白')
                            {                                
                                $('#' + ZEIRISTU_NAME + poststr).val(ui.item.ZEI);        
                                alert("消費税率が10％に更新されました");
                            }
                        }
                    }
		},
                ...
                change : function( event, ui )
                {
                    if(!ui.item)
                    {
                        ...
                        //入力値を取得する
                        var hinmei = document.getElementById(HINMEI_NAME+poststr).value;
                        var tanka = document.getElementById(TANKA_NAME+poststr).value;
                        var suryo = document.getElementById(SURYO_NAME+poststr).value;
                        var tanni = document.getElementById(TANNI_NAME+poststr).value;
                        
                        if(def_hinmei == '' && def_tanka == '' && def_suryo == '' && def_tanni == '')
                        {
                            document.getElementById(HINMEI_NAME+poststr).defaultValue = hinmei;
                            document.getElementById(TANKA_NAME+poststr).defaultValue = tanka;
                            document.getElementById(SURYO_NAME+poststr).defaultValue = suryo;
                            document.getElementById(TANNI_NAME+poststr).defaultValue = tanni;
                            $('#'+ZEIRISTU_NAME+poststr).val(10);

                        }
                        else
                        {
                            if($('#'+ZEIRISTU_NAME+poststr).val() != 10)
                            {
                                $('#'+ZEIRISTU_NAME+poststr).val(10);
                                alert("消費税率が10％に変更されました");
                            }
                        }
                    }    
                }
