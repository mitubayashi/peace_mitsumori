...
function check(checkList, checkNull, notNullType)
{
	...
	for (var i = 0 ; i < checkListArray.length ; i++ )
	{
		var param = checkListArray[i].split("~");
                if(param[0] == "form_jsyTRANSFER_0")
                {
                    if(!kagenformcheck(this))
                    {
                        judge = false;
                    }
                }
...
function kagenformcheck()
{
    var obj = document.getElementsByClassName("form_traTRANSFER[]");
    var judge = true;
    for(var j = 0; j < obj.length; j++)
    {
        ...
        {
            judge = false;
            obj[j].style.backgroundColor = "#ff0000";
        }
        else
        {
            obj[j].style.backgroundColor = "";
        }
    }

    if(!judge)
    {   
        if((event.target.value == "") || (event.type == "submit"))
        {
            alert("値を入力してください。");            
        }
    }
